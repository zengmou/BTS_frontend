                    ...
                    <el-table-column type="expand">
                      <template slot-scope="props">
                        <el-form label-position="left" inline id="demo-table-expand">
                          ...
                          <el-form-item>
                            <label>稿件rebuttal信息：</label>
                            <span>{{ props.row.rebuttals }}</span>
                          </el-form-item>
                        </el-form>
                      </template>
                    </el-table-column>
...
<script>
  ...
  import SideNav from '../common/PCRebuttalSide';
  export default {
    name: "Rebuttal",
    components:{
      NavMenu,
    ...
    created(){
      this.$axios.get('./meeting/review/rebuttal',{
        params:{
        ...
        if(this.markForm.score===0){
          ...
          this.$axios.post('./meeting/review/rebuttal',{
            ...
            if(res.status===200 && res.data===true){
              ...
              this.$message.error('未参与讨论，不能修改复审结果');
            }
          }).catch(error =>{
