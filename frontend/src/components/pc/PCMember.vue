    ...
    <div>
      <el-row>
        ...
        <el-col :span="21">
          ...
          <el-divider content-position="left">尊敬的审稿人，您可以对进行如下会议操作</el-divider>
            ...
            <el-col :span="8">
              <el-card :body-style="{ padding: '0px' }" style="margin: 10px 50px;">
                ...
                <div style="padding: 14px;">
                  <span>想要进行审稿工作？</span>
                  <div>
                    <el-button type="text" class="button" v-on:click="toReview()">前往审稿页面</el-button>
                  </div>
                </div>
              </el-card>
            </el-col>
...
<script>
  ...
  import SideNav from "../common/PCSide";
  export default {
    name: "PCMember",
    components: {
      NavMenu,
    ...
    created(){
      window.localStorage.setItem('authority','PCMember');
      window.localStorage.setItem('meetingId',this.$route.query.id);
      this.$axios.get('./meeting/PC',{
          params:{
    ...
    methods:{
      toSubmit(){
        ...
        else if(this.tableData[0].submitStatus === '投稿已结束'){
          this.$message('当前会议已结束投稿');
        }
        else{
