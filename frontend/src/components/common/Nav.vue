<template>
  <el-menu :default-active="path1" router mode="horizontal" background-color="white" text-color="#222" style="height:100%">
    ...
    <el-drawer :title="title" :visible.sync="drawer" :direction="direction" :before-close="handleClose" :with-header="false">
      ...
      <el-row>
        <el-button @click="addMoney" type="primary" style="margin-top:100px" icon="el-icon-s-finance" round plain>存款</el-button>
      </el-row>
    </el-drawer>
...
<script>
  export default {
    ...
    methods: {
      ...
      addMoney(){
        this.$prompt('请输入存款金额', '确认存款', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          inputPattern: /^[+]{0,1}(\d+)$|^[+]{0,1}(\d+\.\d+)$/,
          inputErrorMessage: '请输入正确的存款金额'
        }).then(({ value }) => {
          this.$axios.get('./bank/account/'+this.id+'/'+value)
            .then(res =>{
              if(res.status===200){
                ...
                this.$message.error('存款出错');
              }
            })
            .catch(error =>{
              this.$message.error('存款请求出错')
            })
        }).catch(() => {
          this.$message({
            type: 'info',
